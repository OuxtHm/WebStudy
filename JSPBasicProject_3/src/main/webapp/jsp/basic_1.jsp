...
<%--
	1장
		...
		3장)
			...
			webapp -> 폴더 : 웹 파일 (jsp, html, css, js)
			   | WEB-INF
			   		| lib -> 라이브러리(.jar)
		...
		1) JSP 구조
			...
			-------------
			구분 -> <% %> : 스크립트릿
				   <%= %> : 화면에 자바 데이터 출력 -> out.println()
				   <%! %> : 선언식 - 멤버변수, 메소드 -> 보안, 소스노출으로인해 사용빈도가 낮다
			실행 : jsp -> servlet
				  | 자바 파일 변경 -> 컴파일 -> 실행
				  | 한 번 변경시에 속도가 늦다(1회성)
		...
		139p
			...
			** page 지시자
				...
				<%@ 지시자 속성="값" 속성="값" ...%>
				-> 첫줄에 코딩
				**-> contentType : 브라우저에 전송방법
						***1 html -> text/html;charset=utf-8
						***2 json -> text/plain,charset=utf-8
				-> errorPage
				**-> isErrorPage
				-> buffer
				**->import
