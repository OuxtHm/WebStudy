...
public class BoardUpdate extends HttpServlet {
	...
	// 화면 출력
		protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			// 1. HTML 전송 -> 브라우저에 알려준다
			response.setContentType("text/html;charset=UTF-8");
			
			// 2. 어떤 브라우저에 HTML을 전송할 지 확인
			...
			out.println("<td width=80%>");
			out.println("<input type=text size=20 name=subject required value="+vo.getSubject()+">");
			out.println("</td>");
			out.println("</tr>");
			...
			out.println("<td width=80%>");
			out.println("<textarea row=5 cols=50 name=content required>"+vo.getContent()+"</textarea>");
			out.println("</td>");
			out.println("</tr>");
