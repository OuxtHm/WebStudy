...
public class MusicMain extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HttpServletResponse : 서버 응답(HTML, XML, JSON)
		doPost / doGet		: 요청에 처리
		...
		*/
		...
		/*
			54p
			HTML 구조
			<!DOCTYPE html> : html 5 버전을 읽어온다.
			<html>	// 문서의 시작
			  	<head>	
			  		화면 출력 (x)
			  		설정 파일
			  		CSS / JavaScript 파일 읽기 -> 파일마다 공통으로 사용
			  		----- 내부 (파일 안에서만 사용)
			  		CSS 설정
			  		JavaScript 설정
			  	</head>
		...
		*/
		...
		// btn-danger btn-info btn-success btn-warning btn-primary
		...
		out.println("<input type=button class=\"btn-sm btn-sucess\" value=Success>");
		...
		out.println("<input type=button class=\"btn-lg btn-sucess\" value=Success>");
		...
		out.println("<input type=button class=\"btn-xs btn-sucess\" value=Success>");
